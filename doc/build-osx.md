...
5.  (Optional) You can also install manod to your path:
...
If you are building `manod` or `mano-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the MANO-Qt.app
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure MANO with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the MANO wallet:
...
You have the choice to build the GUI MANO wallet as a Mac OSX app, described in “How to build the MANO-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the MANO-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find MANO-Qt.dmg inside your MANO folder. Open and install the wallet like any typical Mac app.
