...
VERSION=
commit=false
url=https://github.com/giaki3003/MANO
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the mano, gitian-builder, gitian.sigs, and mano-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/manoproject/mano
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit mano=${COMMIT} --url mano=${url} ../mano/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../mano/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/mano-*.tar.gz build/out/src/mano-*.tar.gz ../mano-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit mano=${COMMIT} --url mano=${url} ../mano/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/mano-*.zip build/out/mano-*.exe ../mano-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit mano=${COMMIT} --url mano=${url} ../mano/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/mano-*.tar.gz build/out/mano-*.dmg ../mano-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit mano=${COMMIT} --url mano=${url} ../mano/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../mano/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/mano-*.tar.gz build/out/src/mano-*.tar.gz ../mano-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../mano/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/mano-*win64-setup.exe ../mano-binaries/${VERSION}
	    mv build/out/mano-*win32-setup.exe ../mano-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../mano/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/mano-osx-signed.dmg ../mano-binaries/${VERSION}/mano-${VERSION}-osx.dmg
	fi
	popd || exit
