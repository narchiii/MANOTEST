...
#ifndef MANO_ZMANOWALLET_H
#define MANO_ZMANOWALLET_H
...
public:
    CzMANOWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMANO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMANO(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MANO_ZMANOWALLET_H
