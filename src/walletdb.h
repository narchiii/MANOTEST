...
public:
    ...
    bool WriteZMANOSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMANOSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMANOSeed_deprecated(uint256& seed);
    bool EraseZMANOSeed();
    bool EraseZMANOSeed_deprecated();

    bool WriteZMANOCount(const uint32_t& nCount);
    bool ReadZMANOCount(uint32_t& nCount);
