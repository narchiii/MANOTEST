...
CzMANOWallet::CzMANOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmano seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMANOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMANOSeed_deprecated()) {
                    LogPrintf("%s: Updated zMANO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmano seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmano wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMANOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMANOCount(nCountLastUsed);
    else if (!walletdb.ReadZMANOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMANOWallet::Lock()
{
    seedMaster = 0;
}

void CzMANOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMANOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMANO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMANOWallet::LoadMintPoolFromDB()
{
...
void CzMANOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMANOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMANOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmanoTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMANOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMANO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmanoTracker which also adds to database
    pwalletMain->zmanoTracker->Add(dMint, true);
...
void CzMANOWallet::SeedToZMANO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMANOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMANOWallet::GenerateDeterministicZMANO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMANOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMANOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
