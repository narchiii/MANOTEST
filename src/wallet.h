...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MANO at the same time
    ONLY_2500 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMANO send
enum ZerocoinSpendStatus {
    ZMANO_SPEND_OKAY = 0,                            // No error
    ZMANO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMANO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMANO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMANO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMANO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMANO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMANO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMANO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMANO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMANO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMANO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMANO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMANO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMANO_SPENT_USED_ZMANO = 14,                      // Coin has already been spend
    ZMANO_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZMANO_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZManoBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZMANOOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmanoAuto) const;
    ...
    */
    ...
    CzMANOWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzMANOTracker> zmanoTracker;
    ...
    void setZWallet(CzMANOWallet* zwallet)
    {
        zwalletMain = zwallet;
        zmanoTracker = std::unique_ptr<CzMANOTracker>(new CzMANOTracker(strWalletFile));
    }

    CzMANOWallet* getZWallet() { return zwalletMain; }
    ...
    void setZManoAutoBackups(bool fEnabled)
    {
