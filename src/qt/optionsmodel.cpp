...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MANO);
    ...
    if (!settings.contains("nAnonymizeMANOAmount"))
        settings.setValue("nAnonymizeMANOAmount", 1000);

    nAnonymizeMANOAmount = settings.value("nAnonymizeMANOAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMANOAmount"))
        SoftSetArg("-anonymizemanoamount", settings.value("nAnonymizeMANOAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mano.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMANOAmount:
            return QVariant(nAnonymizeMANOAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMANOAmount:
            nAnonymizeMANOAmount = value.toInt();
            settings.setValue("nAnonymizeMANOAmount", nAnonymizeMANOAmount);
            emit anonymizeMANOAmountChanged(nAnonymizeMANOAmount);
            break;
        case CoinControlFeatures:
